...
提案会改变公开安装布局、角色 schema、Skill trigger 边界或永久规则时先开 issue。小型、有界修正可以直接提交 pull request。安全报告遵循[安全说明](SECURITY.zh-CN.md)；行为规范问题遵循仅英文的 [Code of Conduct](CODE_OF_CONDUCT.md)。
...
- 没有当前已批准 consumer 时，不新增 workflow、installer、release、compatibility、telemetry、scoring 或 evaluation machinery。
...
WSL 属于 Linux 证据。不能仅凭配置声称 macOS 或 Windows 已验证；必须有精确 commit 上的原生绿色 CI job。Discovery 改变时使用可信新 Codex 会话；当前旧会话不保证热加载。
