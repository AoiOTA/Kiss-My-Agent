...
安装或配置正式发布的插件不需要 Python、Node.js 或其他语言运行时。贡献者工具链与用户路径分开：Git 与 Python 3.11 或更高版本足以运行标准库 validator 和 Setup contract tests。只修改 Plugin 或 Skill 的贡献者不需要安装 Markdown，也不需要在本地构建站点。只有真实 discovery 和 dogfooding 检查才需要 Codex。
...
不要重写其他贡献者的分支、force-push 共享工作，或把 release 准备与无关修复混在一起。
...
1. **项目 instructions 与 roles。** 从当前 checkout 启动可信的新 Codex 会话，给它一项真实且有界的贡献任务。适合委派时，让 `kiss_explorer` 只读调查，为 `kiss_coder` 指定独占的文件 ownership，再让 `kiss_reviewer` 做独立只读审查。确认无关 dirty-tree 改动仍被保留，subprocess failures 仍然可见。
...
2. 通过聚焦 pull request 合入实现。将 plugin manifest version 与 marketplace ref 对齐为 `0.2.0` / `v0.2.0`，同步中英文文档，并要求完整测试套件以及精确 release commit 的 Ubuntu、macOS、Windows 原生 CI 全绿。
3. 完成 fresh-install Smoke、隔离的 `v0.1.0` 到 `v0.2.0` marketplace-upgrade Smoke、fresh-session Skill discovery、无害 role Smokes 与仅依赖 README 的新用户 Pilot。在已安装 cache 真正报告并加载 v0.2.0 前，不得声称 upgrade path 成立。
4. Pull request squash-merge 且精确的 `origin/main` commit 验证通过后，创建不可变的 annotated tag 与 GitHub Release：
...
5. 验证公开 Release 页面与 archives，再在隔离安装和可信新 session 中测试文档给出的用户升级命令：
...
6. 通过 HTTPS 验证 Pages 的两种语言，并通过后续 pull request 在 canonical handoff 中记录精确 commit、CI runs、公开 install/upgrade 结果、真实 Smokes、Pilot 结果和剩余限制。

如果已发布 release 存在缺陷，保留其 tag 并发布新的 patch 版本。绝不 force-push `main`、移动已发布 tag、压掉失败检查，或把 invalid run 重新标记为成功。
