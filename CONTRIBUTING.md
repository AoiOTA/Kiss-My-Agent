...
- Preserve the three owners: the two public switches in `config.toml`, standalone role TOML discovery, and dynamic dispatch in AGENTS. KISS does not own or set the master model/effort. The marker controls remove ownership; it does not authorize resetting an existing value. Config must not enumerate role files.
