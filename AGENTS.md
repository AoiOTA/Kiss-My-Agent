...
- Multi-agent work is available by default, but an explicit user instruction or effective configuration that disables it takes precedence. The current Host-exposed role catalog is open: `kiss_explorer`, `kiss_coder`, and `kiss_reviewer` are shipped only as initial seeds, and roles may be added, renamed, or removed. The master must use only roles that currently exist and dynamically choose whether to delegate, which roles to use, how many agents to involve, and whether work is serial or parallel; there is no fixed workflow or team size. Give each agent only the goal, paths, ownership, invariants, evidence, and stop conditions it needs.
