...
- Multi-Agent V2 is available by default. For non-trivial development, the master uses the professional `kiss_explorer`, `kiss_coder`, and `kiss_reviewer` roles when independence, information gain, risk isolation, or latency exceeds coordination cost; simple tasks may be handled directly by the master. If the user explicitly disables multi-agent work, do not spawn subagents. Give each agent only the goal, paths, ownership, invariants, evidence, and stop conditions it needs.
