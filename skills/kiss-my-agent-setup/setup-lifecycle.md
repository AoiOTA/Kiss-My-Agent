...
9. Read bundled assets only as required by the selected action:
   ...
   - `remove`: read the current, v0.2.6, v0.2.5, and v0.1 role seeds as exact bytes and verify each parsed identity matches its filename before comparing a corresponding existing role.
...
- Delete a bundled role only when its complete bytes equal the current bundled seed or the corresponding exact v0.2.6, v0.2.5, or v0.1 remove-only snapshot. Preserve every other changed or differently identified role and report it.
