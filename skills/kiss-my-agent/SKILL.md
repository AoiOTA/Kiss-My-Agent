...
Start from the assigned outcome, including the user's defining use cases, and what current evidence already settles. When choosing an MVP or interpreting a result, check whether the exercised behavior still requires the capability those use cases depend on. A narrower prerequisite experiment may help, but its success does not turn unexercised requirements into optional future work. Reuse evidence while its relevant implementation, inputs, evaluator, and validity conditions still match; a new policy or parameter identity alone does not invalidate an unchanged mechanism conclusion. A probe earns its cost only if a plausible result would change the next action. When the mechanism is already established, act on it in the real task path instead of repeatedly confirming it or packaging more proof.
