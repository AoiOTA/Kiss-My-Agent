...
20. **v0.1 compatibility and repeat migration.** Copy `tests/fixtures/v0.1-managed-project` to a disposable project. Expect v0.2 check to report its well-formed block as `outdated`; setup must migrate each byte-identical v0.1 role with one native byte-preserving copy from the current bundled seed directly over the same target, never patch, Delete+Add, or text-decode/re-encode it. Re-run setup and expect no role diff. Remove must still recognize byte-identical v0.1 seeds.
