...
- **已定位的触发故障**：此前新delegated explorer收到旧Host技能目录，版本化入口已不存在，首次读取报ENOENT。Paw项目以非受管本地说明指向稳定personal插件源；通用模板不嵌入机器路径。随后master的实际Host目录已更新到`0.2.7+codex.20260910054223`，该入口存在并已成功读取；这是当前master发现路径恢复的证据，不能推广为所有旧child已热加载或所有Host故障解决。
...
- **下一步依据**：加入UMI启发的负载均衡与髋—足偏好后，1000轮候选因持续退化在记录336轮后提前停止，选最新已保存301轮检查点；末50轮非终止奖励裁零99.872%。新PhysX独立64例跌倒24→4但位置通过11→0，静态16例跌倒13→6但到达3→0；MuJoCo后测仍在运行。稳定性改善尚未保住任务能力。已有检查点和保存观测的CPU读出显示18维std均增大，旧裁剪死区部分遮蔽噪声；完整PD映射后的腿目标条件标准差约.36→.91rad。该证据支持检查探索幅度，不证明随机训练零奖主因。保持世界系完整EE pose、单18关节Actor、无外部底盘命令及原性能标准。
