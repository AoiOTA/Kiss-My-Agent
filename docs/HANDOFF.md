...
- softsign250轮及四项后测均已退出0：训练跌倒1,552→841，饱和约10.12M→11.55M；PhysX test8位置和朝向各7/8退步，far完整60秒但位置.5958→.6199 m变差、朝向改善；MuJoCo far22.46→28.72秒仍跌倒，共同窗位置／朝向RMSE及p95全差。root不采用，不自动续训或扫描mapping；后续两份只读分析已完成：实际softsign保住区间内均值与采样概率，hip／后腿响应不再普遍被硬裁剪抹掉，但大latent压缩敏感度、关节变化不均；不能沿用旧解释或据此采用策略。现有±.2 rad是PD目标范围，静态FK抬脚见证和已纠正常量的承重保持都不证明受载动态抬放。root据此执行7秒RR腿within-box抬放诊断：有效PhysX retry2与MuJoCo各3,500物理步、退出0、无跌倒／饱和，但RR只短暂卸载约1–2 mm，未指令的对角FL离地14–16 mm、足中心移动约12 cm，末态机身倾斜约15°并下沉约3.5 cm；MuJoCo RR大腿从4.038秒接地至结束。独立review通过该结果解释；固定pulse不是干净抬放，也不证明整个动作盒不足。软件一致性、CPU响应和减少跌倒不替代任务收益；本次有用取舍是复用实际策略与历史证据，区分机制成立、任务失败及缺失的受载动作证据；尚不证明流程普遍有效，未发现新KMA产品缺陷。避免重复身份机制的具体取舍保留，核心持续自主指导目标仍未完成，整体速度收益未量化，不据本次负面结果新增KMA规则或系统。
...
- 本轮复用同起点控制、既有sampler／checkpoint读出和评估路径，没有新增采集机制；RNG路径和EMA不含朝向的限制保留，未发现需新增KMA产品规则的问题。该来源核对随后完成，新对照状态见下。持续自主指导核心目标未完成，整体速度收益未量化。

- root依据源audit选择 `leg_range_comparison` 两个新匹配250轮学习（腿scale .2／.5）及各四项后测；CPU producer与review通过后，.2控制训练已于UTC2026-09-09T23:38:43.913769启动，实际status的exit仍为null，其余九项未启动。新初始化按真实spec保存并严格加载，初始deterministic PD目标相同、裁剪前角std均.06 rad，六臂与Critic源值保留；裁剪分布、动作历史及优化坐标不同，非纯plant capacity比较，.5也非厂商默认或硬件结论。
- 本次沿用实际初始化／身份消费路径而未引入restore例外，并以新匹配控制区分初始化变化；这是准备与决策层面的具体取舍，启动、agent数及CPU一致性不证明任务收益。实际结果待完整学习与双引擎后测；无已观察的新KMA产品规则缺陷，不改规则，持续自主指导与整体效率收益仍待证据。
