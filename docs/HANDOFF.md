...
- softsign250轮及四项后测均已退出0：训练跌倒1,552→841，饱和约10.12M→11.55M；PhysX test8位置和朝向各7/8退步，far完整60秒但位置.5958→.6199 m变差、朝向改善；MuJoCo far22.46→28.72秒仍跌倒，共同窗位置／朝向RMSE及p95全差。root不采用，不自动续训或扫描mapping；下一步仅只读分析真实latent／mean／std／响应及现有腿动作范围的动态能力证据，无新仿真或扩大范围决定。软件一致性、CPU响应和减少跌倒不替代任务收益；避免重复身份机制的具体取舍保留，核心持续自主指导目标仍未完成，整体速度收益未量化，不据本次负面结果新增KMA规则或系统。
