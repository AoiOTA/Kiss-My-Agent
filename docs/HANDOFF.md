...
- **Paw 当前证据**：full500 与 alpha25 STOP326 均为负结果；normalizer 的闭环对照不支持直接换回旧 N 作为修复。最后的 reward6case 奖励诊断尚在运行，最终读出由 Paw 的实际结果和交接补齐。短诊断、稳定片段与减少跌倒均不证明学习成功；单一 18 关节 Actor、原性能要求、完整位姿及支撑协调目标和临时硬件证据边界保留。
