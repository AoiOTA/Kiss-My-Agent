...
When changing public switches, standalone role discovery, or setup scope, keep `.codex/config.toml`, `.codex/agents/`, plugin metadata, the Agent-native setup references, Configuration, Installation, Testing, both READMEs, and the annotated example consistent. `.codex/config.toml` owns four paths: the paired initial master defaults `gpt-5.6-sol` / `max` and the two public enablement switches; it never enumerates role files. The editable fresh-setup seeds explicitly default to `gpt-5.6-sol` / `high` for explorer and coder and `gpt-5.6-sol` / `xhigh` for reviewer. Plugin cache seeds do not automatically enter the Host catalog. They are not a closed catalog: fresh setup creates missing starters, and every role that already exists is user-owned, so setup and Plugin updates never overwrite, migrate, or version-classify it. Use the existing-role wizard or manual TOML edits for newer model or effort choices. Do not add model fallback, permission fallback, preset matrices, or compatibility wrappers without a current consumer.
